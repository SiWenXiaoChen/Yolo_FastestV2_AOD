...
def compute_loss(preds, targets, cfg, device, clean_img, ori_img):
    balance = [1.0, 0.4]
    ...
    de_loss = criterion(clean_img, ori_img)
    lbox *= 3.2
    lobj *= 64
    lcls *= 32
    loss = lbox + lobj + lcls + de_loss

    return lbox, lobj, lcls, de_loss, loss
