...
class Detector(nn.Module):
    def __init__(self, classes, anchor_num, load_param, is_train, export_onnx = False):
        ...
        if self.is_train == True:
            self.backbone = ShuffleNetV2_dehaze(stage_out_channels, load_param)
        else:
            self.backbone = ShuffleNetV2_dehaze_test(stage_out_channels, load_param)
    ...
    def forward(self, x):
        if self.is_train == True:
            C2, C3, clean_img = self.backbone(x)
        else:
        ...
        if self.export_onnx:
            ...
            return (out_reg_2, out_obj_2, out_cls_2, out_reg_3, out_obj_3, out_cls_3), clean_img
        else:
