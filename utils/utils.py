...
def load_datafile(data_path):
    ...
    str_type_key = ["model_name", "val", "train", "ori_img", "names", "pre_weights"]
...
def evaluation(val_dataloader, cfg, model, device, conf_thres = 0.01, nms_thresh = 0.4, iou_thres = 0.5):
    ...
    for imgs, targets, ori_img in pbar:
        imgs = imgs.to(device).float() / 255.0
        targets = targets.to(device)
        #ori_img = ori_img.to(device).float / 255.0
