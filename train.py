...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/traffic.data',
    ...
    # 数据集加载
    train_dataset = utils.datasets.TensorDataset(cfg["train"], cfg["ori_img"], cfg["width"], cfg["height"], imgaug=True)
    val_dataset = utils.datasets.TensorDataset(cfg["val"], cfg["ori_img"], cfg["width"], cfg["height"], imgaug=False)
    ...
    for epoch in range(cfg["epochs"]):
        ...
        for imgs, targets, ori_imgs in pbar:
            # 数据预处理
            imgs = imgs.to(device).float() / 255.0
            targets = targets.to(device)
            ori_imgs = ori_imgs.to(device) / 255.0

            # 模型推理
            preds, clean_imgs = model(imgs)
            # loss计算
            iou_loss, obj_loss, cls_loss, de_loss, total_loss = utils.loss.compute_loss(preds, targets, cfg, device, clean_imgs, ori_imgs)
            ...
            # 打印相关信息
            info = "Epoch:%d LR:%f CIou:%f Obj:%f Cls:%f de:%f Total:%f" % (
                epoch, lr, iou_loss, obj_loss, cls_loss, de_loss, total_loss)
