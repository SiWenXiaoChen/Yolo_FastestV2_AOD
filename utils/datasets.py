...
def collate_fn(batch):
    img, label, ori_img = zip(*batch)
    for i, l in enumerate(label):
        if l.shape[0] > 0:
            l[:, 0] = i
    return torch.stack(img), torch.cat(label, 0), torch.stack(ori_img)

class TensorDataset():
    def __init__(self, path, ori_path, img_size_width = 352, img_size_height = 352, imgaug = False):
        assert os.path.exists(path), "%s文件路径错误或不存在" % path
        assert os.path.exists(ori_path), "%s文件路径错误或不存在" % ori_path
            ...
            for line in f.readlines():
                ...
                if os.path.exists(data_path):
                    ...
                    if img_type not in self.img_formats:
                        ...
                        self.data_ori_list.append(data_path)
                else:
                    raise Exception("%s is not exist" % data_path)

    def __getitem__(self, index):
        data_orig_path = self.data_ori_list[index]

        data_orig = Image.open(data_orig_path)

        data_orig = data_orig.resize((self.img_size_width, self.img_size_height), Image.ANTIALIAS)
        ...
        return torch.from_numpy(img), torch.from_numpy(label), data_orig
...
if __name__ == "__main__":
    ...
    img, label, ori = data.__getitem__(0)
    print(img.shape)
    print(label.shape)
    print(ori.shape)
